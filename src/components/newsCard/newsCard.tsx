...
export const NewsCard = ({ urlToImage , title , description}: NewsApiResponse) => {
  return (
      <div className="h-72 md:h-96 w-full md:w-1/2 lg:w-1/3 mb-4 lg:mb-0">
        <div className="h-72 md:h-96 block group relative mx-2 overflow-hidden shadow-lg">
          <img src={urlToImage} className="absolute z-0 object-cover w-full h-72 md:h-96" alt="News" />
          ...
          <div className="absolute left-0 right-0 bottom-0 p-6 z-30 transform translate-y-1/2 transition duration-300 h-full group-hover:translate-y-0 delay-100">
            <div className="h-1/2 relative">
              <div className="absolute bottom-0">
                <h2 className="font-bold text-white leading-tight transition duration-300 text-xl pb-6 group-hover:underline">{truncateText(title , 10)}</h2>
              </div>
            </div>
            <div className="h-1/2">
              <p className="text-white pb-4 opacity-0 transition duration-300 group-hover:opacity-100">{truncateText(description , 25)}</p>
